...
export function needAccessToken(
  ...
  return (
    ...
    let accessToken;
    // EMPLOYER api sends accessToken in the authorization header
    if (req.headers.authorization) {
      accessToken = req.headers.authorization.split(' ')[1];
    }

    // worker api sends accessToken in the sessionToken cookie
...
export function checkOrganization() {
  return async (
    ...
    // skip organization checks when request comes from WORKERapi with sessionToken cookie
